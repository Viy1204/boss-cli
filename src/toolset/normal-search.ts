 ...
 */
export function parseRangeArg(raw: string): { min: string; max: string } {
  const parts = raw
    .trim()
    .split(/[-－~～]/)
    .map((s) => s.trim())
...
export function expSliderLabel(index: number): string {
  if (index === 1) {
    return '在校/应届';
  }
 ...
 */
export function expTokenToIndex(token: string): number {
  const t = token.trim();
...
export function ageTokenToLabel(token: string): string {
  const t = token.trim();
 ...
 */
export async function selectNormalSearchExpRange(
  page: Page,
  frame: Frame,
  minToken: string,
  maxToken: string,
 ...
 */
async function readAgeCustomRange(frame: Frame): Promise<{ min: string; max: string } | null> {
  const raw = (await frame.evaluate(`(() => {
...
function ageHiddenToLabel(raw: string): string {
  const n = Number(raw);
 ...
 */
export async function selectNormalSearchAgeRange(
  frame: Frame,
  minToken: string,
  maxToken: string,
): Promise<string> {
  ...
  if (!got || got.min !== minLabel || got.max !== maxLabel) {
    throw new Error(
      `年龄自定义区间没落到「${minLabel}-${maxLabel}」，页面现在显示：${got ? `${got.min}-${got.max}` : '（空）'}。`,
    );
  }
  return `${minLabel}-${maxLabel}`;
}

async function pickAgeCustomDropdown(frame: Frame, index: 0 | 1, label: string): Promise<void> {
  const opened = await evalWithPanelRetry<{ ok: boolean; reason?: string }>(
    frame,
    `(() => {
  ...
  if (!picked.ok) {
    throw new Error(
      `年龄下拉里没有「${label}」这一项。可选：${(picked.options ?? []).join('｜') || '（菜单未展开）'}`,
    );
  }
  await sleepRandom(FILTER_SETTLE_MS.min, FILTER_SETTLE_MS.max);
}
