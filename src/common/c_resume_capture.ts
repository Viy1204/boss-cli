 ...
 */
export async function closeCResumePanel(page: Page): Promise<boolean> {
  for (let round = 0; round < 5; round++) {
    ...
    for (const frame of page.frames()) {
      try {
        const clicked = (await frame.evaluate(CLOSE_C_RESUME_PANEL_SCRIPT)) as boolean;
 ...
 */
async function collectVisibleCResumeIframeHandles(page: Page): Promise<ElementHandle<Element>[]> {
  const found: ElementHandle<Element>[] = [];
  for (const frame of page.frames()) {
...
export async function countVisibleCResumeIframes(page: Page): Promise<number> {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      if (box && box.width > 8 && box.height > 8) {
        ...
        try {
          const state = (await contentFrame.evaluate(C_RESUME_READY_STATE_SCRIPT)) as string;
          if (state === 'content') {
            return true;
          }
          if (state === 'shell') {
...
export function blankBytesPerPixelThreshold(): number {
 ...
 */
export function classifyResumeCaptureDensity(
  byteLength: number,
  pixels: number,
  threshold = blankBytesPerPixelThreshold(),
): { verdict: 'blank' | 'borderline' | 'ok'; density: number } {
