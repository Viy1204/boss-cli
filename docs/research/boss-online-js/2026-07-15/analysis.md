...
- Manual review completed against the 2026-07-14 verified baseline.
- `risk-detection.js` is byte-for-byte identical after normalizing `v10718` to `v10723`; all configured risk-pattern counts are unchanged.
- Zhipin sign vendor and iframe-core are byte-for-byte unchanged. The sign app is byte-for-byte identical after normalizing the version and build hashes.
- Boss bundle chunk `270.js` has one non-security business change for empty job-list initialization; it introduces no new strings or configured risk-pattern hits. Other bundle chunks are unchanged.
- `src/common/boss_page_guards.ts` uses version-independent patterns and still covers the observed security scripts, reporting endpoints, and risk-navigation paths.
- Accept Boss index `v10723`, Boss bundle `v6232`, and Zhipin sign `v5310`; update `src/common/boss_availability.ts` to the archived URLs and SHA-256 hashes.
