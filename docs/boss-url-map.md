...
| `/web/chat/search` | 常规搜索 | `boss search [关键词] [--job] [--city] [--degree|--degree-range] [--school] [--exp|--exp-range] [--age|--age-range] [--status] [--job-hop] [--major]`、`boss preview <姓名>` | `search` 会先检查当前位置，不在该页时直接进入；带关键词时填入搜索框并回车搜索；**每次都先点「清空筛选」(`.reset-btn`) 再按本次参数重设**，筛选条件不跨命令粘连；不传 `--job` 切「不限职位」；城市不在清空范围内（换岗位时平台自己会清）；学历 / 经验 / 年龄是三个独立的单选块（`.degree-ui` / `.exp-list-ui` / `.age-list-ui`，都**不在**「其他筛选」里），院校是多选（`.school-ui`），学历/经验/年龄的「自定义」区间走 `--degree-range` / `--exp-range`（拖 `.experience-select-custom-slider` 的双手柄滑块，按 hidden 的 `min,max` 档位校验纠偏）/ `--age-range`（`.age-custom` 两个下拉，值在 hidden 上：普通年龄存数字、46岁+ 存 10000、不限存 -1）；滑块拖完一律按 hidden 真实档位校验纠偏，且不许拖过对面手柄（两手柄重叠会让按下标取手柄失准）；结果标题里的条件全部实时读页面，不打入参；`preview` 要求当前已在该页且列表已加载，不支持 `--job`。 |
