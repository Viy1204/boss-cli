...
| `/web/chat/search` | 常规搜索 | `boss search [关键词] [--job] [--city] [--degree] [--school] [--exp] [--age] [--status] [--job-hop] [--major]`、`boss preview <姓名>` | `search` 会先检查当前位置，不在该页时直接进入；带关键词时填入搜索框并回车搜索；**每次都先点「清空筛选」(`.reset-btn`) 再按本次参数重设**，筛选条件不跨命令粘连；不传 `--job` 切「不限职位」；城市不在清空范围内（换岗位时平台自己会清）；学历 / 经验 / 年龄是三个独立的单选块（`.degree-ui` / `.exp-list-ui` / `.age-list-ui`，都**不在**「其他筛选」里），院校是多选（`.school-ui`），三个单选块各自的「自定义」滑块/区间未实现；结果标题里的条件全部实时读页面，不打入参；`preview` 要求当前已在该页且列表已加载，不支持 `--job`。 |
