...
export function isBaiduOcrConfigured() {
    return !!(apiKey() && secretKey());
}
let cachedToken = null;
/** 更换密钥后应调用，避免沿用旧 access_token。 */
export function clearBaiduTokenCache() {
    cachedToken = null;
}
...
export async function getBaiduAccessToken() {
    ...
    if (!res.ok || !data.access_token) {
        throw new Error(`百度 OAuth 失败: ${res.status} ${data.error ?? ''} ${data.error_description ?? JSON.stringify(data)}`);
    }
...
export async function baiduOcrImageBase64(imageBase64) {
...
//# sourceMappingURL=baidu_ocr.js.map