...
const BD = '\x1b[1m';
const Z = '\x1b[0m';
function styleBannerLine(content) {
    return `${BG}${FG}${BD}${content}${Z}`;
}
function blankBannerLine(useAnsi) {
    if (useAnsi) {
...
export function printBossInteractiveBanner() {
    const useAnsi = !process.env.NO_COLOR && process.env.TERM !== 'dumb' && (process.stderr.isTTY ?? false);
