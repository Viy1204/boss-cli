...
function shouldCheckFromState(state, now, intervalMs) {
    if (!state) {
        return true;
    }
...
async function writeUpdateCheckState(statePath, state) {
    await mkdir(dirname(statePath), { recursive: true });
    const tmpPath = `${statePath}.tmp`;
    await writeFile(tmpPath, `${JSON.stringify(state, null, 2)}\n`, 'utf8');
    await rename(tmpPath, statePath);
}
export function formatPackageUpdateNotice(result) {
...
export async function printPackageUpdateNoticeIfDue() {
    try {
