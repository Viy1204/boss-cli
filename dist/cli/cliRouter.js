...
import { implChatAction, implLogin, implListCandidates, implListUnreadCandidates, implOpenChatByIndex, implListPositions, implListPositionsWithOptions, implNormalSearch, parseFilterLabels, implOpenChat, implRecommend, implPreview, implRecommendGreet, implSetBaiduCredentials, implBossSearch, implSendMessage, } from '../toolset/index.js';
...
export async function executeCommand(argv) {
    ...
    if (cmd === 'search') {
        const { rest, opts, flags } = parseOpts(tail);
        const usage = '❌ 用法: search [关键词] [--job <岗位关键字>] [--city <城市>] [--degree <学历>|--degree-range <下限-上限>] [--school <院校要求,逗号分隔>] [--exp <经验要求>|--exp-range <下限-上限>] [--age <年龄要求>|--age-range <下限-上限>] [--status <求职状态,逗号分隔>] [--job-hop <跳槽频率>] [--major <专业,逗号分隔>]';
        if (flags.size > 0) {
            die(usage);
        }
    ...
    if (cmd === 'greet') {
        const { rest, opts, flags } = parseOpts(tail);
        const usage = '❌ 用法: greet <姓名> [--job <岗位关键字>]；搜索池: greet [关键词] --index <序号> [--dry-run]';
