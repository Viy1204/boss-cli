...
export async function executeCommand(argv) {
    ...
    if (cmd === 'search') {
        const { rest, opts, flags } = parseOpts(tail);
        const usage = '❌ 用法: search [关键词] [--job <岗位关键字>] [--city <城市>] [--degree <学历>] [--school <院校要求,逗号分隔>] [--exp <经验要求>|--exp-range <下限-上限>] [--age <年龄要求>|--age-range <下限-上限>] [--status <求职状态,逗号分隔>] [--job-hop <跳槽频率>] [--major <专业,逗号分隔>]';
        if (flags.size > 0) {
            die(usage);
        }
