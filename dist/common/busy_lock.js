...
export function acquireBusyLock(command) {
    try {
        mkdirSync(join(homedir(), '.recruit-browser'), { recursive: true });
        writeFileSync(lockFile(), JSON.stringify({ pid: process.pid, command: `boss ${command}`, startedAt: Date.now() }));
    }
    catch {
        /* 锁只是给面板看的提示，写不进去不影响命令 */
    }
}
export function releaseBusyLock() {
    try {
        unlinkSync(lockFile());
    }
    catch {
...
//# sourceMappingURL=busy_lock.js.map