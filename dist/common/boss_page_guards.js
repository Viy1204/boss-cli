 ...
 */
const SHOULD_ALLOW_RISK_NAV = process.env.BOSS_BROWSER_ALLOW_RISK_NAV === 'true' ||
...
function tripPageRisk(page, state) {
    if (riskStateByPage.has(page))
        return;
...
function ensurePageNavigationGuard(page) {
    ...
    const withinWindow = (stamps, now, windowMs) => {
    ...
    page.on('framenavigated', (frame) => {
        ...
        if (riskStateByPage.has(page) || SHOULD_ALLOW_RISK_NAV)
            return;
        bounceAt.push(now);
 ...
 */
function tripIfAlreadyOnRiskPage(page) {
    if (SHOULD_ALLOW_RISK_NAV)
        return;
    let url = '';
    try {
        url = page.url();
    ...
    if (!url || url === 'about:blank' || !isRiskNavigationUrl(url))
        return;
    tripPageRisk(page, {
        kind: 'risk_navigation',
        url,
